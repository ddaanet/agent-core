...
- In `permissions.allow`: `Bash(plugin/bin/prepare-runbook.py:*)`
- Invocation: `plugin/bin/prepare-runbook.py <runbook>` (has shebang, runs directly)
- Requires `dangerouslyDisableSandbox: true` (writes to `.claude/agents/`)
- Do NOT use: `python3 plugin/bin/prepare-runbook.py` (breaks permission match)

### Worktree Operations

**All mutation commands require bypass:** `edify _worktree new`, `merge`, and `rm` write `.claude/settings.local.json` (sandbox allowlist management) and perform operations outside the project root. Always invoke with `dangerouslyDisableSandbox: true`.
...
**Correct pattern:** Use `edify _worktree merge` which handles sandbox bypass. If the tool can't handle the merge, STOP and report. Do not fall back to raw git operations.
...
**Skill instrumentation:** The worktree skill (`plugin/skills/worktree/SKILL.md`) annotates each mutation command with the bypass requirement. Read-only git commands do not need bypass.
