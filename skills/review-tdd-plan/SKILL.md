...
description: Review TDD runbooks for RED/GREEN violations and prescriptive anti-patterns. Use after /plan-tdd before execution.
model: sonnet
requires:
  - TDD runbook at plans/<feature-name>/runbook.md
  - Design document (reference only)
outputs:
  - Review report with violations and recommendations
---
...
Use Grep to find code blocks in GREEN phases:

```bash
grep -A 20 "^\*\*GREEN Phase:\*\*" runbook.md | grep -E "^```python"
```
...
**Problem**: Complete load_config() implementation prescribedRecommendation**: Replace with behavior description
...
```
/review-tdd-plan                          # Review latest runbook
/review-tdd-plan <path>                   # Review specific runbook
/review-tdd-plan <feature>                # Review plans/<feature>/runbook.md
```
...
**Workflow:**
```
/design (TDD mode) → /plan-tdd → /review-tdd-plan → [apply fixes] → prepare-runbook.py → /orchestrate
```

**Automatic review:** /plan-tdd now delegates to tdd-plan-reviewer agent automatically
